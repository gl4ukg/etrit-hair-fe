...
export default function ShopPage({ params }: { params: { locale: 'en' | 'sq' } }) {
  const products = [
    {
      id: 'ys-park-339',
      title: 'Y.S.Park 339',
    ...
    <main className="min-h-screen w-full bg-background text-foreground">
      ...
      <section className="mx-auto w-full max-w-7xl px-6 py-8 lg:px-8">
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {products.map((p) => (
            <Link key={p.id} href={`/shop/${p.id}`} className="no-underline">
              <article className="overflow-hidden rounded-lg border border-white/10 bg-white/5">
                ...
                <div className="border-t border-white/10 bg-black/20 px-4 py-4 text-white">
                  <div className="text-[10px] uppercase tracking-[0.2em] text-white/50">{p.category}</div>
